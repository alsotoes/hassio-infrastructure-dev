...
### Fixed
- Disable HA ingress (ingress: false) - Flask app requires root path `/`
- HA ingress uses path-based routing (`/app/e2762f62_terragui/`) incompatible with Flask
- Access web UI directly via port 5005: `http://homeassistant:5005/`

## [0.1.11] - 2026-09-21

### Fixed
- Copy pyproject.toml to /app/app/pyproject.toml for editable install
- Package needs pyproject.toml at package root for editable install
...
### Fixed
- Bump version to force HA rebuild (cont-init.d bashio::config fix)
- Config values must be retrieved via bashio::config API, not shell env vars

## [0.1.8] - 2026-09-21

### Fixed
- Source bashio library in cont-init.d and services.d scripts
- Fix "bashio::log.info: command not found" error at startup

## [0.1.7] - 2026-09-21

### Fixed
- Add execute permissions to cont-init.d and services.d scripts in Dockerfile
- Fix "Permission denied" error on /etc/cont-init.d/terragui at startup
