...
### Fixed
- Health check: accept 403 as healthy (Garage requires auth for /health endpoint)
- Health check now verifies server responds instead of requiring 200 OK
...
### Fixed
- Fix garage server config loading: use GARAGE_CONFIG env var instead of --config flag
- Fix finish script: remove bashio dependency (not available in service context)

## [0.1.7] - 2026-09-18

### Fixed
- Add openssl package to base image (Alpine) for secret generation
- cont-init.d/garage uses openssl rand for secret generation
